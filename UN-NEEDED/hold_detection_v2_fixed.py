...
def filter_climber_from_mask(
    ...
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        print("[hold_detection] ⚠ Could not open video for person filtering")
        return mask
    ...
    if not ret:
        print("[hold_detection] ⚠ Could not read frame for person filtering")
        return mask
    ...
    with mp_pose.Pose(
        ...
        # FIXED: Check if segmentation_mask exists properly
        if results.segmentation_mask is None:
            print("[hold_detection] ⚠ No segmentation mask available")
            return mask
